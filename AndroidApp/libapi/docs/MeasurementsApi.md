...
[**getLatestStationMeasurement**](MeasurementsApi.md#getLatestStationMeasurement) | **GET** stations/{station_id}/measurements/{relative} | Find the latest measurement of a station.
...
launch(Dispatchers.IO) {
    val result : IndexedMeasurementEntity = webService.getLatestStationMeasurement(stationId, relative)
}
```
...
apiClient.setBearerToken("TOKEN")
val webService = apiClient.createWebservice(MeasurementsApi::class.java)
val measurementId : kotlin.Long = 789 // kotlin.Long | measurement_id is a __unique__ identifier for measurements.  measurement_id is an unsigned integer. 
val stationId : kotlin.Long = 789 // kotlin.Long | station_id is a __unique__ identifier for weather stations station_id is an unsigned integer. 
